...
export class Renderer {
  ...
  event(e){
    ...
    if(e.type==='rescue'){this.bursts.push({x:e.x,y:e.y,color:characterInfo[e.kind].color,life:.32,r:55});this.burst(e.x,e.y,22,[characterInfo[e.kind].color,'#fff4c8','#b8df9a'],150);this.popup('POP!',e.x,e.y-29,'#bb6d73',21);}
    if(e.type==='return'){this.bursts.push({x:e.x,y:e.y,color:'#ff5f82',life:.65,r:155});this.homePulse=1.5;this.burst(e.x,e.y,Math.min(70,e.count*10),['#fff4c3','#ffc4db','#a8dfa7'],210);this.popup(`${e.count} COSMII!`,e.x,e.y-90,'#538d4a',27);}
    if(e.type==='bank'){this.deliveries.push({x:e.x,y:e.y,startX:e.x,startY:e.y,homeX:e.homeX,homeY:e.homeY,kind:e.kind,life:.5});this.popup(`+${e.points}`,e.homeX+45,e.homeY-30,'#b98431',20);}
  ...
  scenery(game,title){
    ...
    c.restore();
    if(!title&&game.rainbow.active){c.save();const rainbow=c.createLinearGradient(0,0,w,h);['#ff80c5','#ffb968','#fff47a','#8cf6b8','#75dfff','#b49bff','#ff87d3'].forEach((color,i)=>rainbow.addColorStop(i/6,color));c.globalAlpha=.8;c.fillStyle=rainbow;c.fillRect(0,0,w,h);c.globalAlpha=.22;c.strokeStyle='#fff';c.lineWidth=24;for(let i=0;i<6;i++){c.beginPath();c.arc(w*.45,h*.5,((this.clock*(this.reduced?0:55)+i*130)%780),0,Math.PI*2);c.stroke();}c.restore();}
  ...
  home(game,h,front=false){const c=this.ctx,s=this.worldToScreen(h.x,h.y),w=340*this.zoom,ih=w*this.assets.home.height/this.assets.home.width;
    ...
    if(front)for(const f of h.attached||[]){if(game.time<f.showAt)continue;const size=43*this.zoom;this.image(this.assets.friends[f.kind],s.x+f.x*this.zoom-size/2,s.y+f.y*this.zoom-size/2,size,size);}
    if(!front){if(this.homePulse>0){c.save();c.globalAlpha=this.homePulse*.3;this.star(s.x,s.y,65*this.zoom,'#fff3a6');c.restore();}}
  }

  friend(f,carried=false,index=0){const c=this.ctx;c.save();c.translate(f.x,f.y+(this.reduced?0:Math.sin(this.clock*3.2+f.phase)*3));
  ...
  world(game,dt,paused){const c=this.ctx;
    ...
    if(p.tap>0){const v=Math.sin(p.tap/.28*Math.PI)*.07;c.scale(1+v,1-v);}const frame=!game.rainbow.active&&p.tap>0&&this.assets.flapFrames?this.assets.flapFrames[Math.min(3,Math.floor((.28-p.tap)/.07))]:this.assets.hero;this.image(frame,-38,-38,76,76);c.restore();
    if(game.rainbow.active){
    ...
    for(const d of this.deliveries){if(!paused)d.life-=dt;const t=clamp(1-d.life/.5,0,1),ease=1-(1-t)**3;d.x=d.startX+(d.homeX-d.startX)*ease;d.y=d.startY+(d.homeY-d.startY)*ease;c.globalAlpha=1-t*.5;const size=50*(1-t*.14);this.image(this.assets.friends[d.kind],d.x-size/2,d.y-size/2,size,size);}c.globalAlpha=1;this.deliveries=this.deliveries.filter(d=>d.life>0);
  ...
  title(){const c=this.ctx,h=this.height;const center=this.width*.51,scale=Math.min(.88,h/390);const positions=[[0,0],[-48,-56],[49,-58],[63,18],[40,85],[-44,80],[-65,15]].map(([x,y])=>({x:center+x*scale,y:h*.55+y*scale}));
  ...
  draw(game,dt,title=false,paused=false){game.view={x:this.camera.x,y:this.camera.y,w:this.width/this.zoom,h:this.height/this.zoom};this.stage=game.stage;if(!paused)this.clock+=dt;if(!title&&!paused)this.followCamera(game,dt);const c=this.ctx;c.save();this.scenery(game,title);if(!this.reduced&&this.shake>0&&!paused)c.translate((Math.random()-.5)*this.shake,(Math.random()-.5)*this.shake);
    ...
    if(!title){const active=game.rainbow.active,x=this.width/2-92,progress=Math.min(1,game.carry.length/RAINBOW_TARGET);this.round(x,12,184,37,13,active||progress===1?'#fff1ab':'#fff9e6e6','#704167',2);this.text(active?`RAINBOW ${Math.ceil(game.rainbow.left)}s`:progress===1?'HOME → RAINBOW':`RAINBOW ${game.carry.length}/${RAINBOW_TARGET}`,this.width/2,26,12,'#8d377c');this.round(x+12,39,160*(active?Math.min(1,game.rainbow.left/12):progress),4,2,'#ef64b7');}
