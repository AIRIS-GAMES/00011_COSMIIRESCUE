import {collaboration} from './collaboration-config.js';
...
export const game=new RescueGame(stage,Math.random,collaboration);
const collaborationUI=new CollaborationUI(game);
...
async function changeScene(action){if(transition.busy||!ready||!bootComplete)return;audio.unlock();clearInput();audio.music(false);audio.stopEffects();await transition.run([renderer.assets.hero,...renderer.assets.friends],action);accumulator=0;last=0;if(document.hidden){if(!paused&&game.state==='playing')pause();}orientationUI();}
...
// Run synchronously in trusted input, before transitions or animation-frame events.
for(const type of ['touchend','click','keydown'])document.addEventListener(type,event=>{if(event.isTrusted)audio.unlock();},{capture:true,passive:true});
...
function show(name){screen=name;collaborationUI.show(name);for(const id of ['title','result','pause-screen','records-screen'])$(id).hidden=name!==id;const playing=name==='playing';$('nav-row').hidden=!playing;$('carry-bar').hidden=!playing;$('pause').hidden=!playing;$('flight-controls').hidden=!playing;$('title-best').textContent=storage.data.best.toLocaleString();}
...
function processEvents(){const sounds=new Set();let rescued=false;for(const e of game.drain()){
  ...
  if(e.type==='gameOver'){
    ...
    $('live').textContent=`${game.rescued}匹を救出。スコア${game.score}。`;if(best){sounds.add('best');renderer.event({type:'best'});}
  }
}
  // Tapping keeps its visual feedback without playing a sound.
  for(const type of sounds)if(type!=='flap')audio.effect(type);
  if(rescued){$('carry-bar').classList.remove('pulse');void $('carry-bar').offsetWidth;$('carry-bar').classList.add('pulse');}
}
function hud(){
...
document.addEventListener('keydown',e=>{
  ...
  if(['Space','ArrowUp','KeyW'].includes(e.code)&&!(e.target instanceof HTMLButtonElement)){e.preventDefault();if(e.repeat)return;if(screen==='playing'&&!paused){if(controlIntro)finishControls();else if(!dockLeft)game.flap();}}
});
...
// Capacitor's built-in scene lifecycle event; keep the existing tap-to-resume flow.
...
function soundLabel(){$('sound').textContent=audio.enabled?'♪':'♪̸';$('sound').setAttribute('aria-label',audio.enabled?'BGMをオフにする（効果音はON）':'BGMをオンにする（効果音はON）');}
...
function frame(now){const dt=Math.min(.06,(now-(last||now))/1000);last=now;
  ...
  collaborationUI.update(screen);hud();audio.setRainbow(game.state==='playing'&&game.powered);audio.music(game.state==='playing'&&!paused&&!transition.busy);renderer.draw(game,dt,screen==='title'||screen==='records-screen',paused||transition.busy||controlIntro>0||dockLeft>0);requestAnimationFrame(frame);
}
try{const [heroImage,friendImages,art,flapFrames,collaborationImages]=await Promise.all([loadImage(hero.src),Promise.all(friends.map(f=>loadImage(f.src))),Promise.all(Object.entries(stage.art).map(async([key,src])=>[key,await loadImage(src)])),Promise.all(hero.frames.map(loadImage)),collaborationUI.loadImages(loadImage)]);renderer=new Renderer($('canvas'),{hero:heroImage,flapFrames,collaborationImages,friends:friendImages,...Object.fromEntries(art)},stage);ready=true;renderer.snap(game);show('title');soundLabel();requestAnimationFrame(frame);await Promise.all([$('boot-logo').decode(),new Promise(resolve=>setTimeout(resolve,Math.max(0,1800-(performance.now()-bootStarted))))]);bootComplete=true;$('boot-screen').hidden=true;$('loading').hidden=true;orientationUI();}catch(error){audio.suspend();ready=false;bootComplete=false;$('boot-screen').hidden=false;$('boot-status').textContent='LOAD FAILED';$('boot-retry').hidden=false;console.error(error);}
