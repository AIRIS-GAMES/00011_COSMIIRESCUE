...
export class GameAudio {
  ...
  stopEffects(){for(const audio of Object.values(this.sfx)){audio.pause();audio.currentTime=0;}this.voices.clear();}
  setRainbow(enabled){
    if(this.rainbow===enabled)return;
    if(enabled)this.normalTime=this.bgm.currentTime;
  ...
  effect(type){
    if(!this.unlocked||this.suspended||globalThis.document?.hidden)return;
    ...
    try{Promise.resolve(audio.play()).catch(error=>{if(this.voices.get(audio)===state)this.mediaError(error,audio);}).finally(()=>{
      if(this.voices.get(audio)!==state)return;
      state.pending=false;
      if(this.suspended||globalThis.document?.hidden){audio.pause();this.voices.delete(audio);}
    });}catch(error){this.mediaError(error,audio);this.voices.delete(audio);}
  }
  music(active){
