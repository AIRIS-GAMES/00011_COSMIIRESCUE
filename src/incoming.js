...
export class IncomingHazards {
  ...
  update(g,dt){
    ...
    if(g.rainbow.active)this.wait=Math.min(this.wait,.42);
    ...
    if(g.rainbow.active)this.wait=.42;
    ...
    if(!g.rainbow.active&&Math.abs(p.vy)>Math.abs(p.vx)*1.3){dx=0;dy=Math.sign(p.vy);}
    const pool=sectionHazards(Math.floor(p.x/1320),Math.floor(p.y/1000),danger||settings.heavy).filter(o=>settings.heavy||!o.directed&&o.damage!==3);
    ...
    const speed=((entry.damage>1?180:135)+(danger?35:0))*settings.speed*(g.rainbow.active?1.5:1);
